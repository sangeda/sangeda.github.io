...
SUPERVISOR_ALIASES=json.loads(ALIAS_PATH.read_text(encoding='utf-8')) if ALIAS_PATH.exists() else {}
...
def supervisor_identity(label):
 ...
 raw=re.sub(r'\s*\((?:sop|itm|muhas)\)\s*def yes(v):return str(v).strip().lower() in {'1','yes','true'}
def main():
...
,'',raw,flags=re.I)
 raw=re.sub(r'^(?:(?:prof(?:essor)?|dr|mr|mrs|ms|pharm)\.?\s*)+','',raw,flags=re.I).strip(' .,-')
 key=raw.casefold()
...
def canonical_supervisor(label):
 ...
 if key in SUPERVISOR_ALIASES:return SUPERVISOR_ALIASES[key],normalized(SUPERVISOR_ALIASES[key])
 if raw and (raw.isupper() or raw.islower()):raw=raw.title()
 return raw,key
 ...
 for i in range(1,5):
  for label in re.split(r'\s+and\s+|[;\n]',str(record.get(f'supervisor_{i}','') or ''),flags=re.I):
   ...
   if key and not key.isdigit() and key!='unnamed supervisor' and key not in seen:
    seen.add(key);result.append({'name':name,'search_key':key})
 return result
